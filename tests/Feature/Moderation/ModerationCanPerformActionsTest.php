...
test('MOD-044: canPerformActions retorna false cuando el caso no está asignado al moderador', function () {
...
test('MOD-045: canPerformActions retorna false cuando el caso ya está completado', function () {
    $publication = createTestPublication($this->vendor, $this->category);
    $case = createModerationCase($publication, [
...
test('MOD-046: canPerformActions retorna mensaje específico para caso apelado', function () {
...
test('MOD-047: canPerformActions retorna true cuando el moderador puede realizar acciones', function () {
    $publication = createTestPublication($this->vendor, $this->category);
    $case = createModerationCase($publication, [
        'assigned_moderator_id' => $this->moderator->id,
        'status' => 'pending',
...
test('MOD-048: canPerformActions incluye información del caso y usuario', function () {
    $publication = createTestPublication($this->vendor, $this->category);
    $case = createModerationCase($publication, [
        'assigned_moderator_id' => $this->moderator->id,
    ]);
